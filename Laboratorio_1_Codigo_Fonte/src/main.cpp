...
GLuint BuildZeros(); // Constrói triângulos para renderização
GLuint BuildOnes(); 
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(500, 500, "INF01047 - 00297033 - Rafael Lacerda Busatta", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    GLuint vertex_array_object_id = BuildOnes();
    GLuint vertex_array_zeros = BuildZeros();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindVertexArray(vertex_array_zeros);
        ...
        //                |          |  |                 |
        //                V          V  V                 V
        glDrawElements(GL_TRIANGLES, 34, GL_UNSIGNED_BYTE, 0);
...
GLuint BuildZeros()
{
    ...
    //    X              Y     Z     W
         // Triângulos externos
         cos(RAD*0)*0.5f,     sin(RAD*0)*0.0f, 0.0f, 1.0f,
         cos(RAD*22.5)*0.5f,  sin(RAD*22.5)*0.7f, 0.0f, 1.0f,
         cos(RAD*45)*0.5f,    sin(RAD*45)*0.7f, 0.0f, 1.0f,
         cos(RAD*67.5)*0.5f,   sin(RAD*67.5)* 0.7f, 0.0f, 1.0f,
         cos(RAD*90)*0.5f,    sin(RAD*90)*0.7f, 0.0f, 1.0f,

         cos(RAD*112.5)*0.5f, sin(RAD*112.5)*0.7f, 0.0f, 1.0f,
         cos(RAD*135)*0.5f,   sin(RAD*135)*0.7f, 0.0f, 1.0f,
         cos(RAD*157.5)*0.5f, sin(RAD*157.5)*0.7f, 0.0f, 1.0f,
         cos(RAD*180)*0.5f,   sin(RAD*180)*0.7f, 0.0f, 1.0f,

          cos(RAD*202.5)*0.5f, sin(RAD*202.5)*0.7f, 0.0f, 1.0f,
         cos(RAD*225)*0.5f,   sin(RAD*225)* 0.7f, 0.0f, 1.0f,
         cos(RAD*247.5)*0.5f, sin(RAD*247.5)* 0.7f, 0.0f, 1.0f,
         cos(RAD*270)*0.5f,   sin(RAD*270)* 0.7f, 0.0f, 1.0f,

         cos(RAD*292.5)*0.5f, sin(RAD*292.5)* 0.7f, 0.0f, 1.0f,
         cos(RAD*315)*0.5f,   sin(RAD*315)* 0.7f, 0.0f, 1.0f,
         cos(RAD*337.5)*0.5f, sin(RAD*337.5)* 0.7f, 0.0f, 1.0f,

        // Triângulos internos
         cos(RAD*0)*0.3f,     sin(RAD*0)*0.0f, 0.0f, 1.0f,
         cos(RAD*22.5)*0.3f,  sin(RAD*22.5)*0.5f, 0.0f, 1.0f,
         cos(RAD*45)*0.3f,    sin(RAD*45)*0.5f, 0.0f, 1.0f,
         cos(RAD*67.5)*0.3f,   sin(RAD*67.5)* 0.55f, 0.0f, 1.0f,
         cos(RAD*90)*0.3f,    sin(RAD*90)*0.58f, 0.0f, 1.0f,

         cos(RAD*112.5)*0.3f, sin(RAD*112.5)*0.55f, 0.0f, 1.0f,
         cos(RAD*135)*0.3f,   sin(RAD*135)*0.5f, 0.0f, 1.0f,
         cos(RAD*157.5)*0.3f, sin(RAD*157.5)*0.5f, 0.0f, 1.0f,
         cos(RAD*180)*0.3f,   sin(RAD*180)*0.5f, 0.0f, 1.0f,

          cos(RAD*202.5)*0.3f, sin(RAD*202.5)*0.5f, 0.0f, 1.0f,
         cos(RAD*225)*0.3f,   sin(RAD*225)* 0.5f, 0.0f, 1.0f,
         cos(RAD*247.5)*0.3f, sin(RAD*247.5)* 0.55f, 0.0f, 1.0f,
         cos(RAD*270)*0.3f,   sin(RAD*270)* 0.58f, 0.0f, 1.0f,

         cos(RAD*292.5)*0.3f, sin(RAD*292.5)* 0.55f, 0.0f, 1.0f,
         cos(RAD*315)*0.3f,   sin(RAD*315)* 0.5f, 0.0f, 1.0f,
         cos(RAD*337.5)*0.3f, sin(RAD*337.5)* 0.5f, 0.0f, 1.0f
    };
    ...
    GLuint vertex_array_zeros;
    glGenVertexArrays(1, &vertex_array_zeros);

    // "Ligamos" o VAO ("bind"). Informamos que iremos atualizar o VAO cujo ID
    // está contido na variável "vertex_array_zeros".
    glBindVertexArray(vertex_array_zeros);
...
GLuint BuildOnes()
{
    ...
    // Este vetor "indices" define a TOPOLOGIA (veja slides 103-110 do documento Aula_04_Modelagem_Geometrica_3D.pdf).
    //
    GLubyte indices[] = { 0,1,2, 0,2,5, 5,2,3, 4,5,3}; // GLubyte: valores entre 0 e 255 (8 bits sem sinal).
